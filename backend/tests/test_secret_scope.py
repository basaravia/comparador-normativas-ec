...
El `.env` dice DÓNDE está el secreto (`X_SECRET_KEY`, `X_SECRET_SCOPE` o `DATABRICKS_SECRET_SCOPE`),
nunca su valor. Estas pruebas fijan la precedencia, que un fallo nombre scope y key sin filtrar
...
def _limpio(monkeypatch):
    for k in ("DATABRICKS_SECRET_SCOPE", "FOUNDRY_AI_TOKEN", "FOUNDRY_AI_TOKEN_SECRET_KEY",
              "FOUNDRY_AI_TOKEN_SECRET_SCOPE", "FOUNDRY_AI_ENDPOINT", "FOUNDRY_AI_API_VERSION",
              "FOUNDRY_AI_DEPLOYMENT", "FOUNDRY_AI_EMBED_DEPLOYMENT"):
...
class TestReferencia:
    ...
    def test_key_y_scope_global(self, monkeypatch, scope_falso):
        ...
        monkeypatch.setenv("FOUNDRY_AI_TOKEN_SECRET_KEY", "foundry-token")
    ...
    def test_scope_propio_gana_al_global(self, monkeypatch, scope_falso):
        monkeypatch.setenv("DATABRICKS_SECRET_SCOPE", "mi-scope")
        monkeypatch.setenv("FOUNDRY_AI_TOKEN_SECRET_SCOPE", "otro-scope")
        monkeypatch.setenv("FOUNDRY_AI_TOKEN_SECRET_KEY", "foundry-token")
    ...
    def test_key_sin_scope_no_lee_y_avisa(self, monkeypatch, scope_falso, caplog):
        ...
        assert scope_falso == []
        assert "DATABRICKS_SECRET_SCOPE" in caplog.text
...
class TestFallosYFugas:
    def test_un_fallo_nombra_scope_y_key_y_cae_al_default(self, monkeypatch, scope_falso, caplog):
        monkeypatch.setenv("DATABRICKS_SECRET_SCOPE", "mi-scope")
        monkeypatch.setenv("FOUNDRY_AI_TOKEN_SECRET_KEY", "no-existe")
        assert settings.get("FOUNDRY_AI_TOKEN", default="") == ""
...
def test_foundry_usa_el_token_del_scope_de_punta_a_punta(monkeypatch, scope_falso):
    ...
    for k, v in {"FOUNDRY_AI_ENDPOINT": "https://mi-recurso.openai.azure.com",
                 ...
                 "DATABRICKS_SECRET_SCOPE": "mi-scope",
                 "FOUNDRY_AI_TOKEN_SECRET_KEY": "foundry-token"}.items():
