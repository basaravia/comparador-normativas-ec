...
# Valor ficticio: el test no llama a ninguna red, solo comprueba el cableado del respaldo.
_CLAVE_DE_PRUEBA = "no-es-una-clave"


class TestLaUINoOrquesta:
    """La regla que hace barata la Fase 2."""

    def test_la_api_no_construye_piezas_del_pipeline(self):
        """La capa de transporte (FastAPI) tampoco reimplementa el pipeline."""
        raiz_api = Path(__file__).resolve().parent.parent / "api"
        fuente = "\n".join(f.read_text() for f in raiz_api.rglob("*.py"))
        constructores = (
        ...
        assert not infractores, (
            f"la API volvió a construir piezas del pipeline: {infractores}. "
            "Debe pedírselas a src/service.py, o el frontend tendrá que reimplementarlas"
        )
