...
def _service_config_from_request(req: StartCompareRequest) -> service.ServiceConfig:
    ...
    """
    # Sin `llm_backend_kind` en la petición (la interfaz mínima no lo envía) se deja el
    # default de `ServiceConfig`, que sale de `LLM_BACKEND` y, sin ella, es "vertex".
    kwargs = {"llm_backend_kind": req.llm_backend_kind} if req.llm_backend_kind else {}
