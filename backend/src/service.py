...
class ServiceConfig:
    ...
    def desde_dict(cls, d: dict) -> ServiceConfig:
        ...
        elif d.get("embed_backend_kind", "").startswith("Foundry"):
            conocidas["embed_backend_kind"] = "foundry"
...
def construir_backend_embeddings(config: ServiceConfig):
    ...
    elif config.embed_backend_kind == "foundry":
        # Deployment de embeddings: `embed_model` explícito, o FOUNDRY_AI_EMBED_DEPLOYMENT.
        spec = ProviderSpec(proveedor=Provider.AZURE, modelo=config.embed_model,
                            batch_size=config.embed_batch_size)
    else:
    ...
    if config.llm_backend_kind in ("openrouter", "groq", "foundry"):
        ...
        if config.llm_backend_kind == "foundry":
            ...
            spec = ProviderSpec(
                ...
                max_retries=3,
                extra={
                    # Los deployments no salen de `/models`.
                    "sin_listado": True,
                    # Opt-in: hay modelos de razonamiento que rechazan una temperatura
                    # distinta de la de fábrica. Se activa solo si el endpoint real lo exige.
                    **({"sin_temperatura": True}
                       if _get_setting("FOUNDRY_OMIT_TEMPERATURE", default="")
