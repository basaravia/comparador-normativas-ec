...
# Almacén de bundles de corridas en memoria
_RUN_BUNDLES: Dict[str, Any] = {}
_RUN_PATHS: Dict[str, service.RunPaths] = {}


def _ejecutar_corrida_background(
    run_id: str,
    handle: RunHandle,
    ...
    try:
        ...
        # Exportar artefactos (Papel de Trabajo de 6 hojas)
        service.exportar(
            results_df,
            rutas,
        ...
        handle.progreso = handle.total
    except Exception as e:
        logger.exception("Error en corrida %s: %s", run_id, e)
        handle.estado = EstadoCorrida.FALLIDO
        handle.error = str(e)


@router.post("/start", response_model=StartCompareResponse)
def start_compare(req: StartCompareRequest, background_tasks: BackgroundTasks) -> StartCompareResponse:
...
async def stream_progress(run_id: str):
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        while True:
            ...
            if h.estado in (EstadoCorrida.COMPLETADO, EstadoCorrida.FALLIDO, EstadoCorrida.CANCELADO):
                break
